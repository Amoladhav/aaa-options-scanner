...
def write_dashboard(result, destination):
    ...
    labels = {'symbol':'Symbol', 'company':'Company', 'group':'Group', 'sector':'Sector', 'bias':'CRS bias',
              'rank':'Rank', 'score':'CRS score', 'percentile':'CRS percentile (higher = stronger)', 'rank_change':'Rank change ↑',
              'ota_field_status':'OTA field diagnostics', 'price_status':'Price age', 'ota_status':'OTA coverage', 'review_status':'Review', 'history_status':'History',
    ...
    for row in result['combined']:
        ...
        body.append(f'<tr data-group="{row["group"]}" data-bias="{row["bias"]}" data-review="{row["review_status"]}" data-percentile="{row.get("percentile", "") if row.get("percentile") is not None else ""}">' + ''.join(cells) + '</tr>')
...
:root{color-scheme:dark}body{margin:0;background:#0c1420;color:#e5eef8;font:15px system-ui,sans-serif}main{padding:28px;max-width:1800px;margin:auto}h1{font-size:32px;margin:10px 0}.eyebrow{color:#73d3c7;letter-spacing:.1em;font-size:12px}.muted{color:#a6b9cd;line-height:1.6}.cards{display:flex;gap:16px;flex-wrap:wrap;margin:24px 0}.card{background:#142337;border:1px solid #30445e;border-radius:12px;padding:18px;min-width:160px}.card b{font-size:30px;display:block}.toolbar{display:flex;gap:12px;flex-wrap:wrap;margin:18px 0}input,select,button{background:#15283d;color:inherit;border:1px solid #41607e;border-radius:5px;padding:9px;font:inherit}a{color:#8edee0}.table{overflow:auto;max-height:65vh;border:1px solid #30445e;border-radius:10px}table{border-collapse:collapse;width:100%}th{position:sticky;top:0;background:#142337;white-space:nowrap}td{padding:12px;border-bottom:1px solid #24364d;white-space:nowrap}tr[data-review=matches_config_unverified]{background:#13352f}details{margin-top:20px}summary{cursor:pointer}button{cursor:pointer}
    ...
    supplied = sum(r.get('tradier_status') == 'supplied_freshness_unverified' for r in result['combined'])
    html += f'<p class="muted">Tradier rows attached: {supplied} / {len(result["combined"])}. Blank quote cells are unavailable, not zero. Check Tradier Status for each row. Attach a saved batch.json or atm-spreads.json using --tradier PATH when generating this report; this page does not fetch quotes.</p>'
    html += '<details><summary>What are OTA field diagnostics?</summary><p>These describe interpretation of source fields, not a trading score: numeric = number; numeric_string = text parsed as a number; missing/null/blank = unavailable; non_numeric = cannot parse; negative_unusable = preserved in raw data but not used for that interpreted metric.</p></details>'
    html += '<p>CRS score measures relative momentum; higher is stronger. Stocks and ETFs are ranked separately. Choose top X% for long research, bottom X% for short research, or both. Ties use midrank percentiles, so counts may differ from exactly X%. These controls change the visible table only; CSV exports, fixed 10% CRS labels and candidate rules stay unchanged.</p>'
    html += '<div class="toolbar"><label>CRS selection <select id="tail"><option value="all">All rows</option><option value="top">Long research: top X%</option><option value="bottom">Short research: bottom X%</option><option value="both">Both tails</option></select></label><label>X (%) <input id="tailPercent" required type="number" min="0.1" max="50" step="0.1" value="10"></label><label>CRS order <select id="crsOrder"><option value="strong">Strongest first</option><option value="weak">Weakest first</option></select></label></div>'
    html += '<div class="toolbar"><input id="search" placeholder="Search symbol or company" aria-label="Search"><select id="group" aria-label="Group"><option value="all">All groups</option><option value="stock">Stocks</option><option value="etf">ETFs</option></select><select id="bias" aria-label="CRS bias"><option value="all">All fixed 10% CRS labels</option><option>long</option><option>short</option><option>neutral</option><option>unranked</option></select><select id="review" aria-label="Review status"><option value="all">All review statuses</option><option value="matches_config_unverified">Matches settings</option><option value="unknown">Unknown</option><option value="below_config">Below settings</option></select><span id="visible"></span></div>'
    html += '<div class="table"><table><thead><tr>' + th + '</tr></thead><tbody>' + ''.join(body) + '</tbody></table></div>'
    html += '<details><summary>Method, freshness and history</summary><p class="muted">21/63/126-session adjusted returns; 50/25/25 weights; cross-sectional z-scores. Price age is a calendar-day limit, not proof of the latest completed exchange session. Retrieval freshness is not quote freshness. Coverage: ' + escape(result['ota_metadata']['coverage']) + '. Positive rank change means improvement since the prior recorded session; history gaps are labeled. Universe changes can alter ranks without a price change. Catalog history retains revisions and selects one result per session; imported legacy history has limited provenance. No backtest or profitability claim.</p><pre>' + escape(json.dumps(result['filters'], indent=2)) + '</pre></details>'
    html += '''<script>
const body=document.querySelector('tbody');const selectors=['search','group','bias','review'].map(id=>document.getElementById(id));
function filter(){const input=document.getElementById('tailPercent');if(!input.checkValidity()){input.reportValidity();return;}const x=Number(input.value)/100,tail=document.getElementById('tail').value;let n=0;for(const row of body.rows){const raw=row.dataset.percentile,p=Number(raw);const inTail=tail==='all'||(raw!==''&&((tail==='top'||tail==='both')&&p>=1-x||(tail==='bottom'||tail==='both')&&p<=x));row.hidden=!(inTail&&row.textContent.toLowerCase().includes(selectors[0].value.toLowerCase())&&selectors.slice(1).every(el=>el.value==='all'||row.dataset[el.id]===el.value));if(!row.hidden)n++;}document.getElementById('visible').textContent=n+' visible';}selectors.forEach(el=>el.addEventListener('input',filter));['tail','tailPercent'].forEach(id=>document.getElementById(id).addEventListener('input',filter));filter();
function orderCRS(){const direction=document.getElementById('crsOrder').value==='strong'?-1:1;const rows=Array.from(body.rows);rows.sort((a,b)=>{const x=a.cells[4].dataset.value,y=b.cells[4].dataset.value;if(x==='')return y===''?0:1;if(y==='')return -1;return direction*(Number(x)-Number(y))||a.cells[0].textContent.localeCompare(b.cells[0].textContent);});rows.forEach(row=>body.appendChild(row));}document.getElementById('crsOrder').addEventListener('change',orderCRS);orderCRS();
